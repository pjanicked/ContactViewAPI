    ...
    public class EmailSender : IEmailSender
    {
        ...
        private async Task SendAsync(MimeMessage mailMessage)
        {
            using (var client = new SmtpClient())
            {
                try
                {
                    client.ServerCertificateValidationCallback = (s, c, h, e) => true;  // This is a hack. Cannot use in production. Needs fix
                    await client.ConnectAsync(_emailOptions.SmtpServer, _emailOptions.Port);
                    client.AuthenticationMechanisms.Remove("XOAUTH2");
